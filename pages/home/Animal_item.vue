<template>
	...
	<view>
		<view class="boxes">
			...
			<view class="" v-for="(item,index) in animal_item" :key="item.shuju.nickname">
				<image :src="item.tupian" mode="" class="print"></image>
				<view class="content">
					<view class="icon">
						
					</view>
					<view class="Animal_name">
						{{item.shuju.nickname}}
					</view>
					<view class="dizhi">
...
<script>
	export default {
		data() {
			// 调用vuex 里的发布数据
			const adopts = this.$store.state.adopts
			console.log(adopts, "领养数据")
		...
		onLoad(options) {
			this.animal_item.push(this.adopts[options.index])

			console.log(this.animal_item, '数据')
		},
